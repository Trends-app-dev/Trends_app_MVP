...
import { setImage, setIsLogin, setSession, setUser, setUserName } from '../../redux/action';
    ...
    if(auxErrors){
      await axios.post('http://localhost:3007/api/v1/users/login', formRegister)
        .then(response => {
          ...
          dispatch(setIsLogin(response.data.access))
          dispatch(setSession(response.data.user.session))
          dispatch(setUserName(response.data.user.userName))
          dispatch(setImage(`http://localhost:3007/${response.data.user.image}`))
    ...
    <div className='flex w-[100%] h-[calc(100vh-80px)] justify-center fixed top-[80px] left-0  '>
      <div className='flex flex-col w-[360px] h-[fit-content] justify-center items-center border-2 border-blue-950 bg-blue-600 shadow-2xl p-[30px] rounded-md mt-[20px] '>
        ...
        <div className='flex gap-[5px] justify-center items-center ' >
            ...
            <Link className='text-xs text-blue-300' to={'/Trends_app_MVP/register'}>Registrate</Link>
        </div>
      </div>
      {
        isModalOpen && (
